document.addEventListener('DOMContentLoaded', function() {
        ...
        function drawTopology(numSU, numSpine, numLeaf, numServers, numNetworkCardsPerServer, serversPerSU, spineLeafDistance, leafNodeDistance, serversPerRow, diagramHeight, nodeRowSpacing) {
            ...
            if (numSU > 2 && numLeaf > 8) {
                nodesGroup.append("text")
                    ...
                    .attr("font-size", "14px")
                    .attr("font-weight", "bold")
                    .attr("fill", "#3b82f6") // 修改为主色调蓝色，与Spine省略号保持一致
                    .text("...");
            }
            ...
            leavesToDisplay.forEach(l => {
                spinesToDisplay.forEach(s => {
                    ...
                    connectionsGroup.append("path")
                        ...
                        .attr("fill", "none")
                        .attr("stroke", "rgba(255, 255, 255, 0.3)") // 增加不透明度
                        .attr("stroke-width", 1.5); // 增加线条粗细
                });
            });
			...
			connectedLeaves.forEach(leafIndex => {
				...
				// Start random traffic flow animations after a short delay
				setTimeout(() => {
				    startRandomTrafficAnimations();
				}, 10000);
			    }
			});
                    }
	...
	function animateConnectionFlow(path) {
	    ...
	    path.selectAll("animate").remove();
	
	    // Determine the total length of the path
	    const totalLength = path.node().getTotalLength();
	
	    // Decide on animation direction (forward or backward)
	    const isForward = Math.random() > 0.5;
	
	    // Choose a random animation style (1: dashes, 2: solid moving color, 3: pulses)
	    const animStyle = Math.floor(Math.random() * 3) + 1;
	
	    // For consistent line appearance
	    path.attr("stroke-width", 2);
	
	    // Apply different animation styles
	    if (animStyle === 1) {
	        // Dashed line moving along the path
	        path.attr("stroke", "rgba(118, 185, 0, 0.8)") // Nvidia green
	            .attr("stroke-dasharray", `${totalLength * 0.05},${totalLength * 0.05}`);
	
	        // Create moving dash animation
	        path.append("animate")
	            .attr("attributeName", "stroke-dashoffset")
	            .attr("from", isForward ? totalLength : 0)
	            .attr("to", isForward ? 0 : totalLength)
	            .attr("dur", "1.5s")
	            .attr("repeatCount", "indefinite");
	    }
	    else if (animStyle === 2) {
	        // Moving color gradient effect
	        path.attr("stroke", "rgba(255, 255, 255, 0.3)");
	
	        // Use gradient animation
	        const gradientId = `flow-gradient-${Math.random().toString(36).substr(2, 9)}`;
	        ...
	        gradient.append("stop")
	            .attr("offset", "50%")
	            .attr("stop-color", "rgba(118, 185, 0, 0.8)");
	        ...
	        gradient.append("animate")
	            ...
	            .attr("from", isForward ? "-100%" : "200%")
	            .attr("to", isForward ? "200%" : "-100%")
	            .attr("dur", "2s")
	            .attr("repeatCount", "indefinite");
	
	        gradient.append("animate")
	            .attr("attributeName", "x2")
	            .attr("from", isForward ? "0%" : "300%")
	            .attr("to", isForward ? "300%" : "0%")
	            .attr("dur", "2s")
	            .attr("repeatCount", "indefinite");
	    }
	    else {
	...
	// Function to start random network traffic animations
	function startRandomTrafficAnimations() {
	    // Get all path elements (connections)
	    const connections = document.querySelectorAll('#diagram-container path');
	    ...
	    const minActiveConnections = 1;  // Minimum number of active animations
	    const maxActiveConnections = 1; // Maximum number of active animations
	    const minAnimDuration = 1000;    // Minimum animation duration (ms)
	    const maxAnimDuration = 1000;    // Maximum animation duration (ms)
	    const minInterval = 1000;         // Minimum interval between new animations (ms)
	    const maxInterval = 1000;        // Maximum interval between new animations (ms)
	    
	    // Track currently animated paths
	    const animatedPaths = new Set();
	    
	    // Function to select a random connection that isn't already animated
	    function selectRandomConnection() {
	        // Filter out already animated connections
	        const availableConnections = connectionsArray.filter(conn => !animatedPaths.has(conn));
	        
	        // If no connections available, return null
	    ...
	    // Function to animate a single connection
	    function animateSingleConnection() {
	        // If we've reached the maximum number of animations, don't add more
	        if (animatedPaths.size >= maxActiveConnections) return;
	        
	        // Select a random connection
	        const connection = selectRandomConnection();
	        if (!connection) return;
	        
	        // Add to tracking set
	        animatedPaths.add(connection);
	        
	        // Create D3 selection from the DOM element
	        const path = d3.select(connection);
	        
	        // Get random duration for this animation
	        const duration = Math.floor(Math.random() * (maxAnimDuration - minAnimDuration)) + minAnimDuration;
	        
	        // Animate the connection
	        animateConnectionFlow(path);
	        
	        // After the animation completes, remove it from tracking and restore original style
	        setTimeout(() => {
	            path.attr("stroke", "rgba(255, 255, 255, 0.3)")
	                .attr("stroke-width", 1.5)
	                .attr("stroke-dasharray", null);
	            
	            // Remove any animation elements that were added
	            path.selectAll("animate").remove();
	            
	            // Remove from tracking set
	            animatedPaths.delete(connection);
	        }, duration);
	    }
    
    // Start with a few random animations
    const initialCount = Math.floor(Math.random() * (maxActiveConnections - minActiveConnections)) + minActiveConnections;
    for (let i = 0; i < initialCount; i++) {
        animateSingleConnection();
    }
    
    // Periodically start new animations
    const animationInterval = setInterval(() => {
        // Random chance to add a new animation
        if (Math.random() < 0.7) { // 70% chance
            animateSingleConnection();
        }
        
        // Every so often, do a "traffic burst" by adding multiple animations
        if (Math.random() < 0.1) { // 10% chance
            const burstCount = Math.floor(Math.random() * 3) + 1; // 1-3 extra connections
    ...
    return animationInterval; // Return the interval ID for stopping later if needed
}
