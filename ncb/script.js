...
// 初始化选项卡
const tabs = ['input', 'results', 'product', 'topology'];
const tabButtons = tabs.map(tab => document.getElementById(`${tab}TabBtn`));
...
tabButtons.forEach((button, index) => {
    button.addEventListener('click', (event) => switchTab(event, tabs[index]));
});
...
function calculateTopology() {
    ...
    // 更新拓扑图
    if (document.getElementById('topologyTab').style.display === 'block') {
        updateTopology();
    }
}